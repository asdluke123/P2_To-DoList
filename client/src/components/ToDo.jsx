...
return(
    <div className="todo">
        ...
        {inFavorite?<span></span>:isEdit ? <Update todo={todo} isToDo={true}/> :<button onClick={() => updateToDo(true,index)}>Update</button>}
        {inFavorite?<span></span>:isEdit ? <button onClick={() => updateToDo(false,index)}>Save Edit</button> :<button onClick={() => deleteToDo(todo._id)}>X</button>}
